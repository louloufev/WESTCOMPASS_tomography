...
def get_vid_compatible(path_vid, t_start, nshot = None):
    ...
    images, data = pyMRAW.load_video(path_vid)
    data['t_start'] = t_start
    savez_compressed(name_path, images = images, t_start = t_start, fps = data['Record Rate(fps)'], image_dim_y = data['Image Height'], image_dim_x = data['Image Width'], NF = data['Total Frame'], skipFrame = data['skipFrame'])
    print(name_path)
